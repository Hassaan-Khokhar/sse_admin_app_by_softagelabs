...
class FeeRepository {
  ...
  /// Reverses a payment by resetting the paid amount to 0 and status to unpaid.
  Future<void> reversePayment(FeeChallan challan) async {
    await _writer.upsert(
      table: _db.feeChallans,
      rowId: challan.id,
      row: FeeChallansCompanion.insert(
        ...
        receivedBy: const Value(null),
        updatedAt: nowTimestamp(),
      ),
    );
  }

  /// Initiates a custom challan for a single student, an entire class, or the whole school.
  Future<int> initiateCustomChallan({
    ...
    if (targetStudentId != null) {
      query.where((s) => s.id.equals(targetStudentId));
    } else if (targetClassId != null) {
      query.where((s) => s.classId.equals(targetClassId));
    }

    final students = await query.get();

    if (students.isEmpty) {
      throw const FeeGenerationException('No matching active students found.');
    }
    ...
    for (final student in students) {
      ...
      await _writer.upsert(
        table: _db.feeChallans,
        rowId: rowId,
        // Since custom challans might happen multiple times a month for a student,
        // we omit conflictTarget which normally deduplicates by (studentId, month, year)
        // for regular fees. Wait, the unique constraint in SQL is strictly by studentId, month, year!
        // So we CANNOT have multiple challans per student per month?
        // Let's check `fee_challans` unique keys.
        row: FeeChallansCompanion.insert(
          id: rowId,
          schoolId: student.schoolId,
          studentId: student.id,
          classId: student.classId!,
          challanNo: 'CH-C$year-${month.toString().padLeft(2, '0')}-$serial-${DateTime.now().millisecondsSinceEpoch % 1000}',
          month: month,
          year: year,
          title: Value(title),
          tuitionFee: const Value(0),
          admissionFee: const Value(0),
          examFee: const Value(0),
          otherFee: Value(amount), // Put amount in otherFee
          arrears: const Value(0),
          totalAmount: amount,
          issueDate: issueDate,
          dueDate: due,
          status: Value(ChallanStatus.unpaid.wire),
    ...
    if (written == 0) {
      throw FeeGenerationException(
        skippedNoClass > 0
            ? 'No challans generated — $skippedNoClass students are not assigned to a class.'
